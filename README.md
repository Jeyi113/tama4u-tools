...
**그림은 LZ77로 묶여 있다.** 로더 스텁 뒤부터 파일 끝까지가 통째로 한
스트림이고, 원본 바이트에서 보이던 스프라이트 헤더는 그 안의 우연이었다.
압축 해제 루틴은 추측이 아니라 **디스어셈블로 읽었다**(vdp-009의 0x1E2,
`xld.w %r1,0x20002bc`가 입력 시작을 알려준다). 내보내는 단위는 전부
16비트 하프워드다.

| 제어 바이트 | 동작 |
|---|---|
| `> 0` | 값 1바이트가 따라옴 → `값:값` 하프워드를 `ctrl+1`회 |
| `< 0` | 하프워드 `-ctrl`개가 그대로 따라옴 |
| `== 0` | 두 바이트 A,B → A가 0이면 종료, 아니면 `(A>>4)+1` 하프워드를 `(A&0xF, B)` 바이트 뒤에서 복사(12비트 음수 거리) |

vdp-009는 27,414B → **60,358B**로 풀리고 스프라이트 **75개**가 픽셀까지
온전하게 나온다(30×36 캐릭터 11프레임, 24×24 아이템 13개, 128×72 배경,
44×52 초상 등). 에디터의 스프라이트 탭이 보여주는 게 이 결과다.

**vdp-001~008은 다른 변형이다.** 제어가 바이트가 아니라 **하프워드**이고
(`ld.h %r2,[%r1]+`), 런 분기가 코어 매뉴얼 표에 없는 옵코드 두 개
(`0x23xx`, `0x27xx`)를 쓴다. 그건 아직 못 읽었으므로 그 파일들은 손대지
않는다 — 억지로 풀면 노이즈가 나온다.
