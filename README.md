...
내용물 레코드도 **압축을 푼 뒤에** 읽는다. 원본 바이트에도 일부가 보이는
건 LZ가 그것들을 리터럴로 내보내기 때문인데, 전부 있는 건 페이로드
쪽뿐이고 스프라이트와 좌표가 맞는 것도 그쪽이다 — vdp-009는 이 차이로
확인 레코드가 7개에서 9개로 늘고, 원본 스캔이 못 보던 식사 둘과 간식의
행선지가 드러난다.

편집은 페이로드에 쓰고 스트림을 다시 만든다. 스트림 앞쪽(헤더, 펌웨어
적응표, 로더 스텁)은 손대지 않으므로 바뀌는 건 패킷 크기와 체크섬뿐이다.
이름은 정상 패킷과 같이 14칸을 전각공백으로 채우고, 행선지는 4바이트
그대로 덮어쓴다 — 둘 다 길이가 고정이라 페이로드 크기가 변하지 않는다.
...
| LZ (009) | 페이로드 동일, 파일은 31,673 → 29,387B (탐욕 매칭이라 매칭 선택이 달라 더 작다) |
