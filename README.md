...
쓰면 `…`가 점 세 개로 풀려 코드↔문자 1:1이 깨지기 때문이다. 남은 빈칸은 오십음도
...
**해독하지 못한 것: 그래픽 대부분.** VDP 하나에 평범한 스프라이트
레코드는 몇 개뿐이고(vdp-009는 5개), 나머지 그림은 프로그램 블롭 안에
다른 형식으로 묶여 있다. `01 FF` 시그니처가 파일 전체에 9개뿐이고 그중
4개는 오탐이다. 에디터의 스프라이트 탭에는 평범한 레코드만 나온다.
