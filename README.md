...
같은 라벨이라도 **기종마다 행선지 코드가 다르다**(iD의 레스토랑 · 식사는
`01010001`, 그 뒤 기종은 전부 `81010201`). 그래서 라벨만으로는 무엇을
고르는지 알 수 없어, 드롭다운과 패킷 목록과 행선지 라벨에 코드를 hex로
같이 적는다. iD가 아이템 인덱스로 남겨두는 바이트는 `**`로 표시해 —
행선지를 바꿔도 그 자리는 건드리면 안 된다는 뜻이다.
