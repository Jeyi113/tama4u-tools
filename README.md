...
**그림은 압축돼 있다.** 로더 스텁 뒤부터 파일 끝까지가 통째로 한
스트림이고, 원본 바이트에서 보이던 스프라이트 헤더는 그 안의 우연이었다.
압축 형식은 추측이 아니라 **디스어셈블로 읽었다**. 내보내는 단위는 둘 다
16비트 하프워드다.

**vdp-001 ~ 008 — 단순 RLE** (제어가 하프워드, 루틴은 0x184)

| 제어 하프워드 | 동작 |
|---|---|
| `> 0` | 하프워드 `ctrl`개가 그대로 따라옴 |
| `< 0` | 하위 바이트가 값, 비트 8-14가 개수 → `값:값`을 `((ctrl>>8)&0x7F)+1`회 |
| `== 0` | 종료 |

**vdp-009 — 제어가 바이트로 바뀌고 역참조가 붙었다** (루틴은 0x1E2)

| 제어 바이트 | 동작 |
|---|---|
| `> 0` | 값 1바이트가 따라옴 → `값:값`을 `ctrl+1`회 |
| `< 0` | 하프워드 `-ctrl`개가 그대로 |
| `== 0` | 두 바이트 A,B → A가 0이면 종료, 아니면 `(A>>4)+1` 하프워드를 `(A&0xF, B)` 바이트 뒤에서 복사(12비트 음수 거리) |

역참조가 붙은 만큼 압축비도 1.8:1 → **2.3:1**로 올라간다.

RLE 쪽의 개수 추출에 쓰이는 두 명령(클래스 1, op2=3)은 코어 매뉴얼 표에
없고 piece-emu도 미정의로 둔다. 그래서 개수는 코드가 아니라 데이터에서
확정했다 — 비트 8-14에 +1이면 아홉 개 VDP가 전부 배포 노트대로 풀린다.

| 파일 | 방식 | 크기 | 스프라이트 |
|---|---|---|---|
| vdp-001 | rle | 27,730 → 47,006 | 56 |
| vdp-005 | rle | 32,748 → 46,938 | 67 |
| vdp-008 | rle | 25,128 → 44,434 | 58 |
| vdp-009 | lz | 27,414 → 60,358 | 75 |

아홉 개 전부 30×36 캐릭터 11프레임과 24×28 UI 아이콘 10개를 공통으로
갖는다. vdp-008이면 쿠리텐 11포즈, 앤젤파이, 운치쿤 액세서리, 별똥별
미니게임, 무지개 하늘 방 — 배포 노트 그대로다.

### 내용물은 통째로 패킷이다

페이로드는 레코드가 흩어진 블롭이 아니라 **완전한 패킷이 줄지어 있는
것**이다 — `TAMAGO` 헤더까지 다 있다. 그래서 컨테이너로 그냥 파싱하면
단독 다운로드가 갖는 모든 필드가 나온다. 에디터는 이것들을 `'vdp'`
경로 단계 아래 정식 하위 패킷으로 노출한다(`[0, 'vdp', 2]`). 덕분에
가격·만복도·친밀도·5스탯·호불호·행선지·이름·스프라이트가 전부 기존
아이템 편집 코드로 읽고 쓰인다.

**행선지가 오해를 부르는 자리가 셋 있다.**

| 자리 | 행선지 코드 | 실제 |
|---|---|---|
| 육성 캐릭터 | `81033300` (타마모리 · 옷) | 아홉 VDP 전부 첫 패킷, 예외 없이 14,664B. ANSI id도 `Violetchi character`라고 적혀 있다 |
| 메뉴 아이콘 세트 | `81042902` (iD L에선 생활용품) | 아홉 VDP 전부 정확히 하나, ~7.5KB. 128×28 바 둘 + 24×28 아이콘 열 |
| 육성 캐릭터 vs 변신 옷 | `81033300` 하나로 둘 다 | 육성 캐릭터는 **예외 없이 14,664B**. Easter 묶음의 8,824B짜리는 가격 1,200의 변신 옷이다 |
| 로딩 아이콘 | `81010101` (냉장고 직행 · 식사) | 데코 피어스가 로딩될 때 뜨는 엠블럼. 늘 ~600B에 24×24 한 장 + 2×2 더미 둘, 이름도 가격도 없다 |
| vdp-009의 빈 슬롯 | `81071f00` (방) | 272B에 2×2 더미 스프라이트뿐 — 내용이 없다 |

앞의 셋은 라벨을 바로잡고, 마지막은 크기로 걸러 `빈 슬롯`으로 적는다.
로딩 아이콘은 vdp-009(별 달린 구름) · vdp-069(별 달린 꽃) ·
easter-pierce(부활절 달걀) 세 파일에 있고, 초기 VDP(001~008)에는 없다.

프로그램 패킷에서 **0x4F는 섹션이 아니라 행선지의 일부**다. `effective_kind`
가 그걸 섹션으로 읽으면 게임이 액세서리가 된다 — 게임센터 코드
`94 02 5b 01`의 0x4F가 0x02이고 0x02가 액세서리 섹션이라, VDP 미니게임에
체형 합성 스트립과 착용 좌표표가 떴다. 프로그램이면 `gm`으로 돌린다.

**행선지를 식별하는 값은 0x4C부터 6바이트다** — 기종/펌웨어 시그니처
2바이트 + 카테고리 코드 4바이트. 코드만 보면 기종이 구별되지 않는다.
레스토랑 · 식사는 iD L·P's·4U에서 똑같이 `81010201`이기 때문이다.
시그니처를 앞에 붙이면 갈린다.

| 카테고리 | iD | iD L | P's | 4U |
|---|---|---|---|---|
| 레스토랑 · 식사 | `cd80 01010301` | `2dc0 81010201` | `8dc0 81010201` | `0101 81010201` |
| 타마모리 · 액세서리 | `0dc0 0102fd00` | `2dc0 81023400` | `8dc0 81023400` | `0101 81023400` |
| 게임센터 · 게임 | `cd80 14020000` | `2dc0 94025b01` | `8dc0 94025b01` | `0101 94025b01` |

iD는 펌웨어까지 갈린다 — 여자아이용 액세서리가 `0dc0`(Lovely Melody)인
게 위 표에 그대로 보인다. 그래서 드롭다운·패킷 목록·행선지 라벨 모두
여섯 바이트를 다 적는다. iD가 아이템 인덱스로 남겨두는 바이트는 `**`로
표시해 — 행선지를 바꿔도 그 자리는 건드리면 안 된다는 뜻이다.

게임 패킷은 시그니처가 P's가 아닐 수 있다 — vdp-008의 게임은 `2D C0`
(iD L)이고 유니코드 이름도 `DL_iDLA_こい▽のぼる.jpg`다. 컨테이너로
파싱하면 이런 게 저절로 해결된다.

### VDP+ — 멀티파트

한 다운로드는 32,768바이트를 못 넘는다. 그래서 큰 묶음은 **VDP+**로
쪼개 배포한다 — 앞 파일이 로더와 설치 행선지를 갖고, 나머지는
`81092900`(통신놀이 · 레시피) 자리에 얹혀 **압축 스트림의 뒷부분만** 싣는다.
뒷부분 파일의 헤더는 로더 껍데기를 복사한 것이고 실제 데이터는 0x100
뒤의 0 패딩을 지나 시작한다.

우리가 가진 Easter(2020)가 그 예다. 앞 파일만으로는 16개가 나오고 마지막
하나가 체크섬이 깨진 채 잘린다. 둘을 이으면 21개가 온전히 나온다 —
육성 캐릭터 셋(EGGBASKET · C RABBIT · NEMUUSA), 식사 셋, 간식 넷,
장난감, 메뉴 아이콘, 액세서리 둘, 변신 옷, 게임, **외출지**, 방 둘.

**경계 규칙.** 세 쌍(Easter · Fairytale · Farmer)을 재보면 규칙이 하나다.

```
스트림 = 1부[0x2BC : 크기-2] + 2부[0x108 : 크기-2]
1부는 언제나 정확히 32,768바이트 → 스트림 용량 32,066B, 나머지가 2부
```

두 군데를 놓치기 쉽다. **패킷 끝 2바이트는 체크섬이지 스트림이 아니다** —
남겨두면 이음매에 쓰레기 2바이트가 끼어 뒤가 전부 노이즈가 된다. 그리고
**2부의 시작은 0x108 고정**이다. 0x100-0x107이 늘 0이라 "첫 비0 바이트"로
찾고 싶어지지만, Easter의 스트림은 진짜로 `00 00`으로 시작한다 — 1부가
토큰 한가운데서 잘렸고 그 둘은 그 토큰의 피연산자다.

이 규칙으로 세 쌍 모두 **체크섬 불량 0개**로 풀리고, 무편집 되쓰기가 두
파일 다 바이트까지 동일하다. 편집하면 다시 압축해 같은 경계로 잘라
두 파일로 저장한다 — 1부는 여전히 32,768바이트다.

### 내용물 교체

VDP 내용물은 다른 다운로드 아이템 파일(.jpg)로 통째로 바꿀 수 있다 —
교체칸에 끌어놓으면 된다. 페이로드를 그 자리에서 고치는 게 아니라 다시
조립하므로 크기가 달라도 된다(4KB 접두부와 패킷 사이 정렬 패딩은 그대로
옮긴다).

가져오는 쪽에서 넘어오는 건 **행선지·가격·만복도·친밀도·5스탯·호불호·
이름·스프라이트**다. 정체성 필드(다운로드 이름, ANSI id, 선언 파일 크기)는
자리 것을 유지한다 — VDP 내용물은 이미 설치된 상태라 이름으로 라우팅되지
않기 때문이다.

카테고리가 다른 아이템을 넣으면 그 자리가 그쪽 카테고리로 바뀐다. 막지는
않되 행선지 코드를 양쪽 다 보여주고 확인을 받는다.

### 되쓰기

편집은 하위 패킷에 쓰고 스트림을 다시 만든다. 스트림 앞쪽(헤더, 펌웨어
적응표, 로더 스텁)은 손대지 않는다. 다시 압축하면 패킷 길이가 달라지므로
0x4A의 선언 크기와 파일 크기를 함께 옮긴다 — 안 그러면 컨테이너가 잘못된
범위를 읽어 체크섬이 엉뚱한 자리에 놓인다.

압축기는 두 방식 모두 구현했다.

| | 무편집 되쓰기 |
|---|---|
| RLE (001~008) | **파일까지 바이트 동일** — 인코더가 원본과 일치한다 |
| LZ (009) | 페이로드 동일, 파일은 31,673 → 29,387B (탐욕 매칭이라 더 작다) |

파이썬과 JS 번들이 같은 바이트를 낸다.
